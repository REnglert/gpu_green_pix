...
__global__ void gcount(Pixels *pic, bool *isEdge, Pixels *result, int xsize, int ysize, int *count){ 
    ...
    //int offset = i*xsize + j; // location of red value
    ...
    for(int i = 0; i < cols; i++){
        for(int j = 0; j < rows; j++){
            int offset = (row+j)*xsize + (col+i); // location of red value
...
main( int argc, char **argv ){
  ...
  char *filename;
    ...
    int *gradient_result = (int *) malloc (xsize*ysize*sizeof(int));
    int *deltaX = NULL; 
    int *deltaY = NULL;  
    ...
    cudaMalloc((void**) &deltaX, xsize*ysize*sizeof(int));
    checkCudaError("malloc deltaX");
    cudaMalloc((void**) &deltaY, xsize*ysize*sizeof(int));
    ...
    //gradient_calc<<<grid, block>>>(d_result_b, d_gradient_result, deltaX, deltaY, xsize, ysize); 
    //checkCudaError("run gradient calc");
    ...
    fprintf(stderr, "Edge Detection Runtime: %f ms\n", elapsed_time_par);
